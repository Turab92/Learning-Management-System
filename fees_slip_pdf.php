...
$pic = 'assets/images/alkahf.png';
 ...
 $sql=mysqli_query($conn,"SELECT fvm.voucher_id, fvm.challan_no, fvm.issue_date, fvm.due_date, fvm.expiry_date, fvm.branch_id,sb.branch_name, ss.FROM_DATE,ss.TO_DATE, css.SECTION_DESCRIPTION, cs.CLASS_DESCRIPTION, fvm.student_id,scs.APPLICANT_NAME,scs.FATHER_NAME, bs.BANK_ACCOUNT_NO,fm.fee_month,fvm.session_id FROM `fee_voucher_master` as fvm JOIN school_branches as sb on fvm.branch_id=sb.branch_id JOIN sessions_setup as ss on fvm.session_id=ss.SESSION_ID JOIN class_setup_section as css on fvm.section_id=css.SECTION_ID JOIN class_setup as cs on fvm.class_id=cs.CLASS_ID JOIN student_current_status as scs on fvm.student_id=scs.STUDENT_ID JOIN banks_setup as bs on fvm.bank_id=bs.BANK_ID JOIN fee_voucher_month as fm on fvm.challan_no=fm.challan_no where fvm.is_print=0  ");
...
// {
// $pdf->SetX($pdf->GetX() - 290);
// $pdf->Cell(25,5,'For The Months:',0,0);
// $x_axis=$pdf->getx();
// $c_width=35;
// $c_height=5;
...
$pdf->Cell(35,5,'Description',1,0,'C');$pdf->Cell(35,5,'Amount',1,1,'C');	
 ...
 $fetch_emp = mysqli_query($conn,"SELECT  fvd.challan_no, fvd.nature_id,np.DESCRIPTION,np.AMOUNT FROM `fee_voucher_detail` as  fvd JOIN nature_payments as np on fvd.nature_id=np.NATURE_ID WHERE fvd.challan_no='$challan_no'");
			...
			$dis_pay=0;
			$arr=mysqli_query($conn,"SELECT challan_no,total_amount,is_receive FROM `fee_voucher_master` WHERE student_id ='$student_id' and is_print='1'");
			$k_count=mysqli_num_rows($arr);
				...
				{
					 mysqli_query($conn,"UPDATE `fee_voucher_master` SET `not_paid`='1' WHERE challan_no='$challan_no12'");
					$arr_d=mysqli_query($conn,"SELECT `discount` FROM `fee_voucher_detail` WHERE challan_no ='$challan_no12'");
					foreach($arr_d as $rows)
					{
						$discount=$rows['discount'];
			...
			$update = mysqli_query($conn,"UPDATE `fee_voucher_master` SET `total_amount`='$total_payment' WHERE voucher_id='$voucher_id'");
...
$payable_by=0;
$print = mysqli_query($conn,"UPDATE `fee_voucher_master` SET `is_print`='1' WHERE voucher_id='$voucher_id'");
}
